...
class FlappyBirdEnv(gym.Env):
    ...
    observation_space = Box(low=-np.inf,high=np.inf, shape=(5,),
                            dtype=np.float32)
   

    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 60}

    def __init__(self, difficulty='easy',render_mode: str | None = None):
        ...
        self.difficulty_levels = {
            'easy': {'pipe_speed': 2, 'pipe_gap': 200},
            'medium': {'pipe_speed': 3.5, 'pipe_gap': 160},
            'hard': {'pipe_speed': 4.5, 'pipe_gap': 130}
        }
        self.difficulty_level={'easy':0,'medium':1,'hard':2}
        self._last_action = 0
        self._score = 0
    ...
    def observation(self) -> ObsType:
        ...
        if next_pipe is None:
            return np.array([self._bird.y,0,self._bird.velocity,0, 0])
        else:
            ...
            return np.array([self._bird.y,gapCenter,self._bird.velocity,vertical_distance, horizontal_distance])
    ...
    def step(self, action: ActType) -> \
        ...
        if add_pipe:
            #self._pipes.append(Pipe(700, self.np_random, speed=self.difficulty_levels[self.difficulty]['pipe_speed'], gap=self.difficulty_levels[self.difficulty]['pipe_gap']))
    ...
    def reset(self, *, seed: int | None = None,
        ...
        # self._pipes = [Pipe(700, self.np_random, speed=self.difficulty_levels[self.difficulty]['pipe_speed'], gap=self.difficulty_levels[self.difficulty]['pipe_gap'])]
        # self._base = Base(700,velocity=self.difficulty_levels[self.difficulty]['pipe_speed'])
