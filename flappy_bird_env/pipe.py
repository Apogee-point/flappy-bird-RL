...
class Pipe(Drawable, Movable):
    def __init__(self, x: int, rng: np.random.Generator, speed: int = 7, gap: int = 200):
        self.x = x
        self.rng = rng

        path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "images", "pipe.png"
        )
    ...
    def set_height(self) -> None:
        ...
        self.top = self.height - self.pipe_top.get_height()  # top height
        self.bottom = self.height + self.gap  # bottom height

    def draw(self, surface: pygame.Surface) -> None:
        surface.blit(self.pipe_top, (self.x, self.top))
        surface.blit(
            self.pipe_bottom, (self.x, self.bottom)
        )  # drawing the top and bottom pipes

    def move(self) -> None:
        self.x -= self.velocity

    # collission detection using mask in pygame
